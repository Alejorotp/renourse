...
export default function GroupsListScreen() {
    ...
    const loadGroups = async () => {
        setLoading(true);
        try {
            console.log('[GroupsListScreen] loadGroups → category', category.id);
        ...
        const maybeGenerate = async () => {
            try {
                ...
                if (filtered.length === 0 && category.courseId) {
                    setLoading(true);
                    console.log('[GroupsListScreen] Aleatorio: generating groups for category', category.id);
                    await ensureRandomGroups({ categoryId: category.id!, maxMembers: category.maxMembers, courseId: String(category.courseId) });
                    await getAllGroups(category.id!);
                }
            } catch (e) {
                console.error('[GroupsListScreen] ensureRandomGroups error', e);
            } finally {
                setLoading(false);
            }
        };
    ...
    useEffect(() => {
        if (category?.id) {
            ...
            console.log('[GroupsListScreen] groups updated for category', category.id, '→', count);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
