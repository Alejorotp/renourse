                ...
                if (courseCats.length > 0) {
                    Promise.all(
                        ...
                        .then((reportsArrays) => {
                            ...
                            console.log('[CourseDetailScreen] Loaded reports:', allReports.length);
                        })
                        .catch((err) => {
                            console.error('[CourseDetailScreen] Error loading reports:', err);
                        });
                } else {
                    console.log('[CourseDetailScreen] No categories found for course, no reports to load');
                    setReports([]);
                }

                fetchAllEvaluations().catch(err => console.error('[CourseDetailScreen] Error fetching evaluations:', err));

            } else {
                getUserEvaluations(String(user.id || ''))
                    .then((evals) => {
                        setUserEvaluations(evals);
                        console.log('[CourseDetailScreen] Loaded user evaluations:', evals.length);
                    })
                    .catch((err) => {
                        console.error('[CourseDetailScreen] Error loading evaluations:', err);
                    });
            }
        }
                                ...
                                <View style={{ gap: 10 }}>
                                    {list.map((cat) => (
                                        <CategoryRow
                                            ...
                                            onPress={() => {
                                                console.log('[CourseDetailScreen] navigate → /groups with category', cat.id);
                                                router.push({
